"""
Task 1.3 - Prediction with Augmented Model

Loads the best augmented model and runs inference on the predict/ split.
Writes artifacts/task03/predictions.csv.
...
def timeout_handler(signum, frame):
    print("[predict_augmented.py] Timeout – saving progress and exiting.")
    sys.exit(0)
...
def main():
    ...
    # Load model — prefer augmented, fall back to base
    model_path = os.path.join(ARTIFACTS_DIR, "best_model_augmented.pt")
    if not os.path.exists(model_path):
        model_path = os.path.join(ARTIFACTS_DIR, "last_model_augmented.pt")
    if not os.path.exists(model_path):
        model_path = os.path.join(ARTIFACTS_DIR, "best_model.pt")
    if not os.path.exists(model_path):
        print("ERROR: No trained model found. Run train_augmented.py first.")
        sys.exit(1)
    ...
    print(f"[predict_augmented.py] Completed in {time.time()-start_time:.1f}s")
